...
with tabs_chromium_history_version:
    ...
    if selected_version:
        position = versions[selected_version]
...
或者 [GoogleChromePortableLauncher](https://sourceforge.net/projects/portableapps/files/Source/Google%20Chrome/Launcher/)
用 7-Zip 打开获得启动器 `GoogleChromePortable.exe`。


### 获取主程序
下载离线安装包 [Win_x64_{position}_mini_installer.exe](https://chromium-browser-snapshots.storage.googleapis.com/Win_x64/{position}/mini_installer.exe)

离线安装包下载好后, 不要运行, 用 7-Zip 打开获得主程序 `chrome.7z`。
...
4. 双击 `GoogleChromePortable.exe (Chrome{milestone}.exe)` 即可启动。
"""
        )
